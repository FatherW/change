...
class dzElm extends PolymerElement {
  ...
  ready() {
      ...
      this.orderManager.getDataByES(this.id).then(res=>{
        //   this.order = res;
    ...
    //       this.product = res;
    //       console.log('Prod',this.pid,this.product);
    //       this.pRes = res;
    //   });
...
async loadData(res){
    ...
    try{
        await this.userManager.getDataByCache(this.order['buyerId']).then(buyer=>{
          ...
          console.log('Is Show 1',this.user['id'],this.order['buyerId'],this.product['ownerId'],this.isShow);
    
        await this.userManager.getDataByCache(this.order['sellerId']).then(seller=>{
            this.seller = seller;
    
        });
        console.log('Is Show 2',this.order,this.user['id'],this.order['buyerId'],this.product['ownerId'],this.order['productId'],this.isShow);
    
        await this.prodManager.getDataByCache(this.order['productId']).then(product=>{
            this.product = product;
        });
        console.log('Is Show 3',this.user['id'],this.order['buyerId'],this.product['ownerId'],this.isShow);
            if (this.order.exchangeId) {
        ...
        console.log('Is Show 4',this.user['id'],this.order['buyerId'],this.product['ownerId'],this.isShow);
    } catch(e){};
...
accept() {
    ...
    let com = 'change-message';

    try {
        if (this.product['isFree'])
            com = 'change-free-message';
        else
            com = 'change-message';
    } catch(e){}
    ...
    let dzPopup = document.createElement(com);
...
// to, subject, content
        ...
        console.log('Order',that.order,that.order.exchangeId);
        ...
        that.tokenManager.getDataByCache(token).then(res=>{
        ...
        Dazzle.getContent('/email-template/sendorderemail.html').then(html=>{
            ...
            content = content.replace('[[sellerImage]]',that.seller['head']);
            ...
            let cfm = confirm('給買家評個分嗎？');
            if (cfm){
                token = that.buyerToken();
                location.href="/rating?"+token;
            } else {
                location.reload();
            }
