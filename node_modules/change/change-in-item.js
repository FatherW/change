...
class changeInItem extends PolymerElement {
  ...
  constructor() {
    ...
    let user = store.get('change-user') || null;
        ...
        document.addEventListener('set-owner',e=>{
        

            let owner = e.detail.ownerId || null;
            this.ownerId = owner;
            this.postStatus = true;
            this._search();
        ...
        document.addEventListener('my-favor',e=>{
            console.log('My Favor');
            let ownerId = e.detail.ownerId;
            let items = e.detail.items;
            this.isPurchase = null;
            this.displayitem(items);
        });
        document.addEventListener('my-stock',e=>{
        ...
        document.addEventListener('my-order-history',e=>{
            console.log('My Stock');
            let ownerId = e.detail.ownerId;
            this.ownerId = ownerId;
        ...
        document.addEventListener('my-all',e=>{
            let owner = e.detail.ownerId || null;
            this.ownerId = owner;
            let items = e.detail.items;
            this.isPurchase = null;
            console.log('My All Stock',items);
...
async getMyAllStock(ownerId){
    console.log('Max Price');
    let that = this;
    let dbUrl = "https://search-dazzle-n4kjr6eecpjbne65ypunb42isu.ap-southeast-1.es.amazonaws.com/change.product/_count?size=0";
    let params = {
        "query": {
            "bool": {
                "must":[
...
_search(){  
    ...
    if (this.postStatus!=null)
        must.push({ "match": { "postStatus":   this.postStatus  }});
    if (this.isPurchase!=null)
        must.push({ "match": { "isPurchase":   this.isPurchase  }});

    console.log('Post Status',this.postStatus,must,this.ownerId);
    ...
    if (this.isSelection!=null)
        must_not.push({ "match": { "isExchange":   true  }});
...
customElements.define('change-in-item', changeInItem);
