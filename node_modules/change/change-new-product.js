...
class dzElm extends PolymerElement {
  ...
  try{
    ...
    try{
        let user = store.get('change-user') || null;
        console.log('Is Own',this.product.ownerId,user.id);
        if (!user)
            return false;
        else if (user.isAdmin)
            return true;
        else if (this.product['ownerId'].toString() === user.id.toString())
            return true;
        else
            return false;        
    }catch(e){}
...
initProduct(prod) {
    ...
    let isEdit = this.isOwn();
    if (isEdit){
        let ceb =document.createElement('change-edit-bar');
        let container = this.shadowRoot.querySelector('.outer');
        container.insertBefore(ceb, container.firstChild);
        // shadow.appendChild(ceb);
    }
...
loadProduct(pid) {
    ...
    return new Promise(function(resolve, reject) {
        that.dataManager.getDataByES(pid).then(res=>{
                resolve(res);
        },err=>{
            reject();
        });
...
_findseller() {
    ...
    if (user == null) {
        ...
        Dazzle.componentPopup(popup, '500px', '900px');
