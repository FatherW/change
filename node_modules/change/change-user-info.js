import { PolymerElement, html } from '/node_modules/@polymer/polymer/polymer-element.js';
import '/node_modules/@vaadin/vaadin-dialog/vaadin-dialog.js';
import '/node_modules/@vaadin/vaadin-select/vaadin-select.js';
import '/node_modules/@vaadin/vaadin-button/vaadin-button.js';
import '/node_modules/@vaadin/vaadin-text-field/vaadin-text-field.js';
import '/node_modules/@vaadin/vaadin-text-field/vaadin-email-field.js';
import '/node_modules/@vaadin/vaadin-text-field/vaadin-number-field.js';
import {Dazzle} from '/node_modules/@dazzle/dz-dazzle/dz-library.js';
import "/node_modules/@cwmr/paper-tags-input/paper-tags-input.js";

import './change-user-banner.js';
import './change-control-pannel.js';
import './change-add-product.js';
import './change-user.js';
import './change-district.js';
import './change-shop.js';
import './change-ch-pw.js';
import '/node_modules/@dazzle/dz-dazzle/css/main-style.js';
import {changeInItem} from './change-in-item.js';



class dzElm extends PolymerElement {
  static get properties () {
    return {
        user:{
            notify: true,
            readOnly: false
        }
    };
  }

  constructor() {
    super();
    this.dataInit();
    // this.updateAllAlias();   
  }

  updateAllAlias(){
    let userManager = new DataPackage('user');
    let user = store.get('change-user') || null;
    let fileManager = new AwsPackage(user);
    userManager.getAllData().then(res=>{
                Dazzle.getContent('/user.html').then(html=>{
                    res.forEach(item=>{
                        let parser = new DOMParser();
                        let elm = parser.parseFromString(html, "text/html");
                        let meta = document.createElement('meta');
                        meta.setAttribute('name','user-id');
                        meta.setAttribute('content',item['id']);
                        elm.head.appendChild(meta);
                        // console.log('All User',elm.documentElement.innerHTML);
                        let isFanpage = item['isFanpage'] || false;
                        if (!isFanpage) {
                            fileManager.saveHtml('user/'+item['username'],"<!DOCTYPE html>"+elm.documentElement.outerHTML);                         
                            item['alias'] = 'user/'+item['username'];    
                        } else {
                            fileManager.saveHtml('page/'+item['username'],"<!DOCTYPE html>"+elm.documentElement.outerHTML);                         
                            item['alias'] = 'page/'+item['username'];    
                        }
                        console.log('All User',item,isFanpage);
                        userManager.saveDataWithCache(item['id'],item);
                    });
                });
    });
    
  }

  dataInit(){
    let user = store.get('change-user') || null;
    Dazzle.subUser = user;
    console.log('Change Info',user,Dazzle.subUser);
    
    if (user) {
        this.fileManager = new AwsPackage(user);
        this.fileManager.listFile('user/').then(res=>{
            this.noAlias = res.length;
            user['alias'] = user['username'] + this.noAlias;
        });
    }
    Dazzle.uid = "change";
    console.log('Change User',Dazzle.uid);
    let elm = document.head.querySelector('meta[name=user-id]');
    let id;
    if (window.location.hash)
        id = window.location.hash.substring(1);
    else if (elm)
        id = elm.getAttribute('content');
    else if (user)
        id = user['id'];
    else {
//        alert('用戶未登入或不能讀取用戶資料');
//        location.href="/index.html";
    }

    this.id = id;
    console.log('User Info',user,this.id);
    // if (user){
    //     Dazzle.loadChangeComponent('change','change-add-product');
    // }
    this.userManager = new DataPackage('user');
   

  }

  getMyPageList(){
    let query = {
        "term":{
            "fansAdmin": this.user['email']
        }
    };
    this.userManager.searchDataByQuery(query).then(res=>{this.listPage = res; console.log('List Page',this.listPage);});

      
  }
  ready() {
      super.ready();
      let loc = location.search;
      let user = store.get('change-user') || null;
      this.subUser = user;
      let shadow = this.shadowRoot;
      let that = this
  
      shadow.querySelector('change-shop').ownerId = this.id;
      this.userManager.loadData(this.id).then(res=>{
        this.user = res;
        console.log('User Info',this.user);
        this.renew();
    },
    err=>{
        // alert("不能確認此頁身份，請聯絡管理員");
        // location.href = '/index.html';
    });

    this.myAll();


      this.edit = "編輯";
      document.addEventListener('areachange', e => {
        let free = e.detail.isFree;
        let shadow = this.shadowRoot;
        let items =  shadow.querySelectorAll('.product-button');
        if (free == true) {
            items.forEach(item=>{item.classList.add('is-free')});
//            item.style.backgroundColor = "#4CAF50";
            
        } else {
            items.forEach(item=>{item.classList.remove('is-free')});
//            item.style.backgroundColor = "#F0846C";
        }
    });

    document.addEventListener('select-district',e=>{
        let district = e.detail.district;
        this.user['area'] = district;
    });
     
}
renew(){
    let user = this.user;
    this.regisDate = "剛剛";
    this.userManager.saveDataWithCache(this.user['id'],this.user);
    let t = this.user['createTime'] || new Date().getTime();
    if (t)
        this.regisDate = Dazzle.parseTime(t);                  

    // this.score = Math.round(this.user.score * 10) / 10;
    if (user['numberRate'])
        this.score = Math.round(user['score']/user['numberRate']);
    else
        this.score = 0;

    this.user['history']  = this.user['history'] || [];
    this.user['banner'] = this.user['banner'] || "https://www.gettv.hk/Website%20Design/profile%20page/img/profile%20banner/company-profile-banner.png";
    // this.getGoodsNum(user['id']);
    this.isFan = this.isFanAdmin();
    this.own = this.isOwn();
    console.log('Is Fan',this.isFan);
    this.getMyAllStockCount(this.id);
    this.getMyAllStock();
    // this.execute();
    console.log('My User',this.user);
    this.getMyPageList();
    try{

        if(this.user['isFanpage']) {
            console.log('Users',this.user,this.shadowRoot.querySelector('#ch-pw'));
            this.shadowRoot.querySelector('#ch-pw').style.display="none";
        }

    }catch(e){

    }

    // if (!this.user['alias'])
    //     this.reAlias(0);



}
execute() {
    // console.log("Entered");
    // let shadow = this.shadowRoot;
    // try{
    //     customElements.whenDefined('vaadin-select').then(function() {
    //         const district = [{
    //             id: '中西區',
    //             name: '中西區'
    //         }, {
    //             id: '灣仔區',
    //             name: '灣仔區'
    //         }, {
    //             id: '東區',
    //             name: '東區'
    //         }, {
    //             id: '南區',
    //             name: '南區'
    //         }, {
    //             id: '油尖旺區',
    //             name: '油尖旺區'
    //         }, {
    //             id: '深水埗',
    //             name: '深水埗區'
    //         }, {
    //             id: '九龍城區',
    //             name: '九龍城區'
    //         }, {
    //             id: '黃大仙區',
    //             name: '黃大仙區'
    //         }, {
    //             id: '觀塘區',
    //             name: '觀塘區'
    //         }, {
    //             id: '葵青區',
    //             name: '葵青區'
    //         }, {
    //             id: '荃灣區',
    //             name: '荃灣區'
    //         }, {
    //             id: '屯門區',
    //             name: '屯門區'
    //         }, {
    //             id: '元朗區',
    //             name: '元朗區'
    //         }, {
    //             id: '離島區',
    //             name: '離島區'
    //         }, {
    //             id: '北區',
    //             name: '北區'
    //         }, {
    //             id: '大埔區',
    //             name: '大埔區'
    //         }, {
    //             id: '沙田區',
    //             name: '沙田區'
    //         }, {
    //             id: '西貢區',
    //             name: '西貢區'
    //         }];
    //         shadow.querySelector('vaadin-select').renderer = function(root) {
    //             // Check if there is a list-box generated with the previous renderer call to update its content instead of recreation
    //             if (root.firstChild) {
    //                 return;
    //             }
    //             // Create the <vaadin-list-box>
    //             const listBox = document.createElement('vaadin-list-box');
    //             district.forEach(function(item) {
    //                 const vaadinItem = document.createElement('vaadin-item');
    //                 vaadinItem.textContent = item.name;
    //                 listBox.appendChild(vaadinItem);
    //                 vaadinItem.setAttribute('value', item.id);
    //             });
    //             // update the content
    //             root.appendChild(listBox);
    //         };
    //         // shadow.querySelector('vaadin-select').addEventListener('change',e=>{
    //         //     console.log('Target',e.target.value);
    //         // });
    //     });
    
    //     // shadow.querySelector('#name1').addEventListener('blur',e=>{
    //     //     this.reAlias(0);
    //     // });
    // } catch(e){

    // }

    


}
async getMyAllStock(){
    let that = this;
    let ownerId = this.id;
    let isFree = store.get('isFree') || false;
    let dbUrl = "https://search-dazzle-n4kjr6eecpjbne65ypunb42isu.ap-southeast-1.es.amazonaws.com/change.product/_search";
    let params = {
        "query": {
            "bool": {
                "must":[
                    {"match":{"isFree":isFree}},
                    {"match":{"ownerId":ownerId}},
                    {"match":{"postStatus":true}}
                ]
              
            }
          }
    };
    await new Promise((resolve, reject) => {
        Dazzle.postData(dbUrl,params).then(res=>{
            console.log('My All Stock',res.hits.hits);
            let items=[];
            // if (!Array.isArray(res.hits))
            //     res.hits =[res.hits];
            res.hits.hits.forEach(item=>{
                console.log('My All Stock',item);

                items.push(item._source['id']);
            });
            Dazzle.dzFire('set-owner',{
                detail:{
                    'ownerId': this.id,
                    'items':items
                }
            });
            resolve(res);            
        },err=>{
            resolve([]);
        });              
    }); 
}


async getMyAllStockCount(ownerId){
    let that = this;
    let dbUrl = "https://search-dazzle-n4kjr6eecpjbne65ypunb42isu.ap-southeast-1.es.amazonaws.com/change.product/_count";
    let params = {
        "query": {
            "bool": {
                "must":[
                    {"match":{"isFree":false}},
                    {"match":{"ownerId":ownerId}}
                ]
              
            }
          }
    };
    await new Promise((resolve, reject) => {
        Dazzle.postData(dbUrl,params).then(res=>{
                console.log('Max Price',res.count);
                that.goodsNum = res.count;
               resolve(res.count);            
        },err=>{
            that.goodsNum = 0;
            resolve(0);
        });              
    }); 
}

getGoodsNum(){
    this.getMyAllStockCount(this.user['id']);
    // .then(no=>{
    //         console.log('Count',no);
    //          this.user['goodsNum']=no;  
    //     },err=>{
    //         this.user['goodsNum']=0;
    //     });     
}
checkAlias(){
    if (!this.user['alias']) {
        let cfm = confirm('你還未有用戶頁網址，要開嗎?');
        if (cfm)
            this.changeAlias(this.user['username']);
    } else {
        Dazzle.getContent('/user/'+this.user['alias']).then(res=>{
            return;
        },err=>{
            this.saveUserPage();
        });
    }

}
_changeAlias(){
    this.changeAlias(this.user['username']);
}
changeAlias(name){
    
    Dazzle.getContent('/user/'+name).then(res=>{
        let name2 = prompt('路徑已被人使用，建議在名稱後加數字作為路徑',name);
        this.changeAlias(name2);
    },err=>{
        this.user['alias'] = name;
        // this.saveUserPage();
        this.save();
        // this.$.profile1.render();
        // alert('更新用戶頁成功');
    });
    // let alias = this.user['alias'] || this.user['username'] || null;
    // alias = alias.replace(/^\s+|\s+$/gm,'')
    // alias = alias.replace(/ /gm,'-');
    // alias = alias.replace(/\//gm,'-');                
    // this.userManager.matchData({"alias":alias}).then(res=>{
    //     if (res.length>1) {
    //         this.canSave = false;
    //         let cfm = alert('閣下用戶名稱已被使用，請即改名, 否則不能儲存');
    //     }
    //     else
    //         this.canSave = true;
    // });
}
reAlias(i){
    let alias = this.user['username'];
    alias = alias.replace(/^\s+|\s+$/gm,'')
    alias = alias.replace(/ /gm,'-');
    alias = alias.replace(/\//gm,'-');
    if (i>0)
        alias = alias+'-'+i;
    this.userManager.matchData({"alias":alias}).then(res=>{
        if (!res.length)
            this.user['alias'] = alias;                    
        else{
            i++;
            this.reAlias(i);
        }
        console.log('Change Alias',res,res.length);
    });
}
_uploadFile(e) {
    let user = store.get('change-user') || null;
    let permit = false;
    try {
        if (!this.user['isFanpage']) {
            if (this.id != user['id'])
                permit = false;
            else
                permit = true;
        }
        else {
            console.log('Click',this.user,user['id'],this.user['fansAdmin'],permit);

            if (this.user['fansAdmin'].indexOf(user['id'].toString())>-1)
                permit=true;
            else
                permit=false;
        }        

    } catch(e){
        if (this.id === user['id'])
            permit = true;
        console.log('Click',this.user,permit);

    }
    console.log('Click',this.user,permit);

    if (!permit) {
        alert('你沒有權限更新圖片');
        return;
    }

    let shadow = this.shadowRoot;
    let that = this;
    let elm= e.target;
    // Dazzle.subUser = user;
    
    Dazzle.loadComponent('dz-file-management','dz-sub-img-stock');
    Dazzle.activeElement = elm;
    let dialog = shadow.querySelector('vaadin-dialog');
    let dzPopup = document.createElement('dz-sub-img-stock');
    dzPopup.subUser = user;
    dzPopup.dialog = dialog;
    Dazzle.componentPopup(dzPopup,'90%','90%');
    elm.selected = function(url,originUrl,smallUrl){
        let id = elm.getAttribute('id') || null;
        if (id==="mp") {
            elm.style.backgroundImage ="url("+originUrl+")";
            that.user['banner'] = originUrl;
        }

        if (id ==="dp") {
            elm.setAttribute('src',smallUrl);
            that.user['head'] = smallUrl;
        }

        dialog.opened = false;
    }
    
    // this.dialog = dialog;
    // this.curElm = e.path[0];
    // let user = store.get('change-user') || null;
    // id = document.head.querySelector('meta[name=user-id]').getAttribute('content') || null;
    // let userInfo = document.querySelector('change-user-info');
    // // let keyword = userInfo.shadowRoot.querySelector('#vb').innerHTML;
    // console.log('User',user['id'],id);
    // if (user['id'] == id) {
    //     Dazzle.popup(dialog, 'change', 'change-gallery', '90%', '90%');
    // } else {
    //     console.log("Will not be able to open gallery!");
    // }

    // console.log('saved', bucket)
}
toggle() {
    //GET BANNER INFO//
    let that = this;
    // let userBan = document.querySelector('change-user-banner');
    let mp = this.shadowRoot.querySelector('#mp').style.backgroundImage;
    bannerSrc = mp.replace('url(', '').replace(')', '').replace(/\"/gi, "");
    let headSrc = this.shadowRoot.querySelector('#dp').src;
    // console.log("document:", document);
    // console.log("omgomg", userBan);
    // console.log("omgomg1", bannerSrc);
    // console.log("omgomg2", headSrc);

    let user = store.get('change-user') || null;
    console.log("UserID", this.user['id']);
    let id = this.user['id'];

        let shadow = this.shadowRoot;
        let text = shadow.querySelector('#vb');
        if (text.innerHTML == "編輯") {
            let name = shadow.querySelector('#name').innerText;
            let email = shadow.querySelector('#email').innerText;
            let area = shadow.querySelector('#area').innerText;
            shadow.querySelector('#name').innerHTML = "<vaadin-text-field id='name1' value='" + name + "'></vaadin-text-field>";
            shadow.querySelector('#area').innerHTML = "<vaadin-select id='area1' value='" + area + "' label=''></vaadin-select>";
            this.execute();
            shadow.querySelector('#email').innerHTML = "<vaadin-email-field id='email1' value='" + email + "'disabled></vaadin-text-field>";
            text.innerHTML = "提交";
        } else {
                // let url = "https://41khtanrje.execute-api.ap-northeast-1.amazonaws.com/prod/Dazzle-elasticSearchController";
                let name = shadow.querySelector('#name1').value;
                let area = shadow.querySelector('#area1').value;
                let email = shadow.querySelector('#email1').value;
                console.log("User", user);
                user['username'] = name;
                user['area'] = area;
                user['banner'] = bannerSrc;
                user['head'] = headSrc;
                let alias = user['username'].trim();
                alias=alias.replace(/ /gi,"-");
                console.log('Alias',alias);
                store.set("change-user", user);
                this.userManager.matchData({"username":user['username']}).then(res=>{
                    if (res.length>1)
                        user['alias'] = alias+res.length;
                    else
                        user['alias'] = alias;
                    that.user = user;
                    store.set("change-user", user);
                    that.saveUserPage();
                });


                // Restore text
                shadow.querySelector('#name').innerHTML = user['username'];
                shadow.querySelector('#area').innerHTML = user['area'];
                shadow.querySelector('#email').innerHTML = user['email'];
                text.innerHTML = "編輯";




             
        }
    

}
save(){
        console.log('This User',this.user);
        this.user['username'] = this.shadowRoot.querySelector('#name1').value;
        // this.user['area'] = this.shadowRoot.querySelector('#area1').value;
        // this.user['banner'] = this.$.mp.style.backgroundImage;
        // this.user['head'] = this.$.dp.getAttribute('src');
        this.user['alias'] = 'user/'+this.user['username']+'-'+this.noAlias;
        this.userManager.saveDataWithCache(this.id,this.user);
        // store.set('change-user',this.user);
        this.saveUserPage();
        alert('成功儲存');
        // location.reload();

        // this.$.render();
        // location.reload();
//                    console.log('This User',this.user);
}
saveUserPage(){
    let that = this;
    let user = store.get('change-user');
    let parser = new DOMParser();
   
    // fileManager.saveFile('user/testing','1234');

        let html = document.createElement('html');
        let head = document.createElement('head');
        let body = document.createElement('body');

        let meta = document.createElement('meta');
        meta.setAttribute('name','user-id');
        meta.setAttribute('content',this.user['id']);
        Dazzle.getContent('/user.html').then(content=>{
            console.log('Save User Page');
            let doc = parser.parseFromString(content, "text/html");
            head.innerHTML = doc.head.innerHTML;
            body.innerHTML = doc.body.innerHTML;

            head.appendChild(meta);
            html.appendChild(head);
            html.appendChild(body);
            let str = html.outerHTML;
            let fileManager = new AwsPackage(user);
            fileManager.saveHtml(that.user['alias'],str);
        });



}
_saveJson() {
    let that = this;
    let url = "/json/user-" + this.user['id'] + ".json?id=" + new Date().getTime();
    Dazzle.getContent(url).then(res => {
        res = JSON.parse(res);
        that.fileManager.saveFile('json/user-' + this.user['id']+ '.json', JSON.stringify(res));
        alert("用戶資料已更新!");

    });

}

isOwn(){

    console.log('Is Own',Dazzle.subUser,this.user);
    let permit = false;

    try {

        
        if ( Dazzle.subUser.isAdmin)
            permit = true;

        if (this.isFan)
            permit = true;
        else {
            if (this.id == Dazzle.subUser['id'])    
                permit = true;
        } 
    } catch(e){permit = false;}


    console.log('Is Own',Dazzle.subUser,this.user,permit);
    return permit;
}
_changePw(){
    let dzPopup = document.createElement('change-ch-pw');
    dzPopup.dialog = this.shadowRoot.querySelector('vaadin-dialog');
    Dazzle.componentPopup(dzPopup,'300px','400px');

}
_add(){


    let product = {};
    // store.set('new-product-id',null);
    // store.set('new-product',product);
    console.log('Who am I',this.user);
    store.set('product-owner',this.user);
    let dzPopup = document.createElement('change-add-product');
    // dzPopup.myOwner = this.user;
    dzPopup.isFan = false;
    dzPopup.owner = this.user;
    dzPopup.product = null;
    dzPopup.dialog = this.shadowRoot.querySelector('vaadin-dialog');
    Dazzle.componentPopup(dzPopup,'95%','95%');
    // location.replace("/add-product.html");

}
myFavor(e){
console.log('My Favor');
try{
    let target = e.target;
    this.shadowRoot.querySelectorAll('.product-button').forEach(item=>{
        item.classList.remove('active');
    });
    target.classList.add('active');
} catch(e){};

    Dazzle.dzFire('my-favor',{
        detail:{
            'ownerId': this.user['id'],
            'items': this.user['likeProduct']
        }
    });
}

myOrderHistory(e){
    let target = e.target;
    this.shadowRoot.querySelectorAll('.product-button').forEach(item=>{
        item.classList.remove('active');
    });
    target.classList.add('active');
    Dazzle.dzFire('my-order-history',{
        detail:{
            'ownerId': this.user['id']
        }
    });
}
myAll(e){
    console.log('Set Onwer');
    Dazzle.dzFire('set-owner',{
        detail:{
            'ownerId': this.id
        }
    });
    try {
        let target = e.target;
        this.shadowRoot.querySelectorAll('.product-button').forEach(item=>{
            item.classList.remove('active');
        });
        target.classList.add('active');
        this.getMyAllStock();

    }   catch(e){
        console.log('err',e);
    }

} 
myStock(e){
    let target = e.target;
    this.shadowRoot.querySelectorAll('.product-button').forEach(item=>{
        item.classList.remove('active');
    });
    target.classList.add('active');
    Dazzle.dzFire('my-stock',{
        detail:{
            'ownerId': this.user['id']
        }
    });
}

_addTags(e) {
    let myTags = this.shadowRoot.querySelector('#myTags');
    console.log('My Tags',myTags);

    let val = this.shadowRoot.querySelector('#inputTags').value;
    // console.log('Key Down', e);

    if (e.code === "Enter") {
        this.userManager.getDataByCache(val).then(item=>{
            // console.log('Tags',myTags.tags);
            // let tags = myTags.tags;
            // tags.push(val);
            // myTags.tags = tags;
            // this.$.myTags._addTag(val);'
            myTags.addTag(val);

        },err=>{
            alert('沒有這個用戶, 請重新輸入');
        });

        this.shadowRoot.querySelector('#inputTags').value = "";
    }
    //console.log("arrayResult:", myTags.tags);
}

isFanAdmin(){

    let user = store.get('change-user');
    try {
        console.log('Is Fans Admin',this.user['fansAdmin']);

        if (!this.user.isFanpage)
         return false;
        if (user.isAdmin)
             return true;
        console.log('Is Fans Admin',this.user.fansAdmin,Dazzle.subUser);
        console.log('Is Fans Admin',this.user.fansAdmin.indexOf(Dazzle.subUser['email']));

        if (this.user.fansAdmin.indexOf(Dazzle.subUser['email'])>-1)
            return true;
 
    }catch(e){
        return false;
    }        
   

    
    return false;

}
  static get template () {
    // Template getter must return an instance of HTMLTemplateElement.
    // The html helper function makes this easy.
    return html`
     
        
    <style>
        * {
            font-family: sans-serif, Microsoft JhengHei;

        }

        #mp {
            /* padding-top: 17%; */
            background-size: cover;
            background-position: center center;
            background-repeat: no-repeat;
            /* background-image: url('./Website Design/profile page/img/profile banner/company-profile-banner.png'); */
            height: 327px;
        }

        #offer {
            margin: 0 auto;
            width: 100%;
            display: flex;
            padding: 10px 0;
            height: auto;
            background-color: white;
        }

        .in {
            padding-right: 1%;
            padding-left: 1%;
            margin-left: 25px;
        }

        #dp {
            width: 70px;
            height: 70px;
            border-radius: 9999999999999px;
            margin: 18% 0%;
            padding-top: 0px;
        }

        #user {
            padding-top: 1%;
            width: 100%;
        }

        #Uname {
            font-size: 1.5em;
            font-weight: 800;
            color: black
        }

        #check {
            color: gray;
        }

        #buttonM {
            display: flex;
            margin-right: 25px;
        }

        .buttons {
            width: 115px;
            text-align: center;
        }

        h6 {
            margin: 10% 0%;
            font-size: 15px;
        }

        .ret {
            font-size: 3em;
            text-align: center;
            color: red;
        }


        @media only screen and (max-width: 768px) {
            .buttons {
                width: 70px;
            }
        }


        @media only screen and (max-width: 640px) {
            #offer {
                width: 100%;
                margin-bottom: 10px;
            }

            .in {
                padding-right: 0;
                padding-left: 0;
                margin-left: 0;
            }

            #dp {
                width: 60px;
                height: 60px;
            }

            #user {
                margin-top: 10px;
                margin-left: 10px;
            }

            #Uname {
                font-size: 1.1em;
            }

            #buttonM {
                margin-right: 0;
            }

            .buttons {
                width: 60px;
                margin-top: 5px;
            }

            .ret {
                font-size: 2em;
            }
        }
    </style>
   <style>
    *,
    p,
    h5 {
        color: black;
        font-family: sans-serif, Microsoft JhengHei;
    }

    a {
        text-decoration: none;

    }

    #info {
        padding: 8px;
        margin-left: 100px;
    }

    #left {
        width: 80%;
        float: left;
    }

    .inf {
        line-height: 2.5;
        color: grey;
    }

    .infotext {
        color: black;

    }

    #right {
        /* width: 75%; */
        display: inline-flex;
        align-content: flex-end;
    }

    /* #top {
        margin-left: 8%;
        margin-right: 8%;
        margin-top: 2%;
        padding-top: 2%;
        padding-left: 2%;
        background-color: white;
        height: auto;
    } */

    .grid-container {
        display: grid;
        grid-template-columns: 2.5fr 1fr;
        background-color: white;
        width: 100%;
        margin: 0 auto;
    }

    h5 {
        font-size: 1em;
        color: black;
    }

    #container {
        float: left;
        background-color: #FFFBF2;
        width: 100%;
        padding-top: 30px;
        padding-bottom: 30px;
        margin-bottom: 10px;
    }

    /* .buttons {
        margin-block-start: 1.67em;
        margin-block-end: 1.67em;
        margin-right: 100px;
        display: flex;
    } */
    a:hover{
            color:orange;
        }
    .v-buttons {
        margin-block-start: 1.67em;
        margin-block-end: 1.67em;
        margin-right: 100px;
        display: flex;
    }
    @media (max-width: 640px) {
        .grid-container {
            display: block;
            height: 340px;
            width: 100%;
            margin: 0px;
            padding: 0px;
            margin-bottom: 10px;
        }

        #container {
            padding-top: 0;
            padding-bottom: 0;
            margin-bottom: 0;
        }

        #info {
            margin-left: 0;
        }

        .buttons {
            display: block;
            margin-block-start: 0em;
            margin-block-end: 0em;
            margin-left: 20px;
        }
       
    }
    .shop_panel {
        padding: 0px;
        padding-bottom: 30px;
        padding-left: 3%;
        padding-right: 3%;
        width: 94%;
        background-image: url(../img/background/3.jpg);
    }
    .shop_panel vaadin-button {
        background:white;
        float:right;
    }
    .active {
        background:#F0846C !important;
    }
    .is-free.active {
        background:#4CAF50 !important;
    }

    .things-grid {
        display: grid;
        height: 100%;
        max-width:600px;
        grid-row-gap: 15px;
        padding-bottom: 50px;
        grid-column-gap: 15px;
        grid-row-gap: 20px;
        grid-template-columns: repeat(2, 1fr);
        background-color: white;
    }

</style>
  
<change-user-banner>

<div id="mp" type="banner" on-click="_uploadFile" style$="background-image:url({{user.banner}})">
</div>
<div id="offer">
    <div class="in">
        <img id="dp" type="profile" on-click="_uploadFile" src$={{user.head}}>
        <!-- src="img/symbol/s5.png" -->
    </div>
    <vaadin-dialog></vaadin-dialog>
    <div id="user" class="in">
        <span id="Uname">{{user.username}}</span>
        <br />
        <span id="check"> 註冊日期:{{regisDate}}前</span>
    </div>
    <div id="buttonM">
        <div class="in buttons">
            <h6>上貨數</h6>
            <span class="ret" id="gNum">{{goodsNum}}</span>
        </div>
        <div class="in buttons">
            <h6>購買數</h6>
            <span class="ret" id="buy">{{user.history.length}}</span>
        </div>
        <div class="in buttons" id="threeWords">
            <h6>評分/(次數)</h6>
            <span class="ret" id="rate">{{score}}</span><span class="numRate">({{user.numberRate}})</span>
        </div>
    </div>

</div>


</change-user-banner>
        <template is="dom-if" if="{{user.isAdmin}}">
            <change-control-pannel></change-control-pannel>
        </template>
        <div class="grid-container">
            <div id="left">
                <div id="info">
                    <h5>賣家資料</h5>
                    <template is="dom-if" if="{{own}}">
                        <div class="things-grid">
                                <label>名稱:</label>
                                <vaadin-text-field id="name1" value="{{user.username}}"></vaadin-text-field>                       
                                <label>地區:</label>
                                <change-district id="area1" value="{{user.area}}" style="width:200px"></change-district>
                                <label>電郵:</label>
                                <vaadin-email-field id="email1" value$="{{user.email}}" disabled></vaadin-email-field>
                                <label>用戶頁:</label>
                                <vaadin-text-field id="profile1" style="min-width:350px;" value$="https://www.gettv.hk/user/{{user.alias}}" disabled></vaadin-text-field>
                        </div>
                        <template is="dom-if" if="{{isFan}}">
                            <h5>專頁管理員清單 (請輸入欲加管理員之註冊電郵)</h5>
                            <div>
                                <vaadin-text-field id="inputTags" on-keydown="_addTags"></vaadin-text-field>
                                 <paper-tags-input width="100%" id="myTags" tags="{{user.fansAdmin}}"></paper-tags-input>
                            </div>
                        </template>

                        <!-- <vaadin-button id="ap" style="margin-right: 10px;" on-click="_changeAlias">建立用戶頁</vaadin-button>  -->
                    </template>

                    <template is="dom-if" if="{{!own}}">

                    <div class="things-grid">
                            <label>名稱:</label>
                            <span class="infotext" id="name">{{user.username}}</span>                   
                            <label>地區:</label>
                            <span class="infotext" id="area">{{user.area}}</span>
                            <label>電郵:</label>
                            <span class="infotext" id="email">{{user.email}}</span>
                            <label>用戶頁:</label>
                            <span class="infotext" id="profile">https://www.gettv.hk/user/{{user.alias}}</span>

                    </div>
                    </template>
                    <template is="dom-if" if="{{listPage.length}}">
                        <p class="inf">管理專頁:
                            <template is="dom-repeat" items="{{listPage}}">
                                <a href$="/page#{{item.id}}">{{item.username}}</a>
                            </template>


                    </template>

                </div>
            </div>
            <div id="right">
                <div class="v-buttons">


                    <template is="dom-if" if="{{own}}">
                        <vaadin-button id="ch-pw"  style="margin-right: 10px;" on-click="_changePw">更改密碼</vaadin-button>
                        <vaadin-button id="ap" style="margin-right: 10px;" on-click="_add">新增貨品</vaadin-button>
                        <vaadin-button id="sb" on-click="save">儲存</vaadin-button>
                    </template>
                    
                </div>
            </div>
        </div>
        <div class="shop_panel">
        <template is="dom-if" if="{{own}}">

            <vaadin-button class="product-button" on-click="myFavor">我的喜愛清單</vaadin-button>
            <vaadin-button class="product-button" on-click="myOrderHistory">過去交易</vaadin-button>
            <vaadin-button class="product-button" on-click="myStock">下架貨品</vaadin-button>
            <vaadin-button class="product-button" id="myAll" on-click="myAll">上架貨品</vaadin-button>

        </template>
            <change-shop></change-shop>
        </div>

<!--        <change-user></change-user>  -->
    </div>
   
    <vaadin-dialog></vaadin-dialog>
    `;
  }
}

// Register the element with the browser.
customElements.define('change-user-info', dzElm);

