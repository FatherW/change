...
import "./change-item-tag.js";
import './change-confirm.js';
import './change-things-panel.js';

import 'https://d25k6mzsu7mq5l.cloudfront.net/node_modules/@polymer/iron-icons/iron-icons.js';
import 'https://d25k6mzsu7mq5l.cloudfront.net/node_modules/@polymer/iron-icon/iron-icon.js';
// import '@polymer/iron-icons/iron-icons.js';

// import "https://d25k6mzsu7mq5l.cloudfront.net/bower_components/dz-dazzle/css/medium-editor.js";

class changethings extends PolymerElement {
    ...
    ready() {
        ...
        let that = this;
        var json;
        let url = "/json/product-" + this.id + ".json?id=" + new Date().getTime();
        let path = "json/product-" + this.id + '.json';

        this.dataManager.getDataByCache(this.id).then(res => {
            console.log('Things', res['productname'], res['id']);
            this.product = res;
    ...
    async isShow() {
        try {
            let ownerId = this.product['ownerId'];
            console.log('Is Show',this.user,this.product);
            if (this.user.isAdmin)
                return true;
            else if (this.user.id == ownerId)
                return true;
            else {
                let userId = String(this.user.id);

                await this.dataManager.getDataByCache(ownerId).then(res => {
    ...
    saveProductPage() {
        let that = this;
        let alias = this.product['productname'].trim();
        alias = alias.replace(/ /gi, "-");
        this.product['alias'] = alias;
        this.detailUrl = '/product/' + this.product['alias'];
    ...
    _reallydelete() {
        ...
        this.dataManager.removeData(this.id).then(res => {
            this.remove();
    ...
    userdata(all) {
        ...
        let ownerId = all['ownerId'];

        console.log('Alias', all['alias']);
        // if (!all['alias']) {
            // this.saveProductPage();
        // } else
            // this.detailUrl = '/product/' + all['alias'];
            this.detailUrl = '/product.html#'+all['id'];
        // this.userUrl = "/user.html?" + ownerId;
        ...
        this.shadowRoot.querySelector('change-user-tag').loadData(all['ownerId']);

        this.shadowRoot.querySelector('change-user-tag').setAgo(all['createTime']);
        ...
        if (user) {
            ...
            if (boolean || user['_id'] == all['ownerId']) {
                // <change-things-panel ownerid="{{product.ownerId}}" pid="{{product.id}}"></change-things-panel>
                let ctp= document.createElement('change-things-panel');
                ctp.setAttribute('ownerid',all.ownerId);
                ctp.setAttribute('pid',all.id);
                shadow.querySelector('.shouldhide').style.visibility = 'visible';
                shadow.appendChild(ctp);
            }

            if (user['adminId']) {
                if (user['adminId'].indexOf(user['id']) >= 0) {
                    shadow.querySelector('.shouldhide').style.visibility = 'visible';

                }
            }
        }
        if (this.product['isExchange'])
            this.shadowRoot.querySelector('.attr span').innerHTML = '貨品交換中';
    ...
    static get template() {
        ...
        <style>
            ...
            * {
            ...
            .attr span{
                background-color: rgb(240, 132, 108);
                color:white;
                padding:2px;
                position:absolute;
                top:0px;
        ...
        <template is="dom-if" if="{{isSelection}}">
            <div id="overlay" on-click="_selectProduct"></div>
        </template>
        ...
        <div id="deleteall">
            ...
            <div id="templike">
                <change-like id$="{{id}}" product$="{{product}}"></change-like>
            </div>
            <div　on-click="_loadProduct">
                ...
                <div id="price" class="price"><span>$</span> {{product.price}}
                </div>
            </div>
            <change-item-tag>
...
customElements.define('change-things', changethings);