    ...
    //   });
      ...
      document.addEventListener('select-product',e=>{
         ...
         if (cfm){
            ...
            this.prodManager.getDataByES(id).then(res=>{
                try {
                    console.log('Exchange',id,res,res.replyArray);
...
//        let token = this.generateToken(15);
        ...
        let orderId = this.saveOrder();
        let token = Dazzle.getEncryptToken();
        let json = {
            "order":orderId,
            "seller":this.seller['id'].toString(),
            "buyer": this.user['id'],
        ...
        this.tokenManager.saveDataWithCache(token,json);
        ...
        Dazzle.getContent('/email-template/sendcontactemail.html').then(html => {
            ...
            Dazzle.sendEmail( to, '【Get Change】你有新的訊息', html);
            // Dazzle.sendEmail("henry@01power.net", '你有新的訊息', html);
            this.dialog.opened = false;
            alert('電郵已發出');
            location.reload();
            // this.tokenManager.getDataByCache(token).then(res=>{
...
try{
    ...
    if (this.product['exchangeId'])
        this.prodManager.getDataByCache(this.product['exchangeId']).then(res=>{
            res.isExchange = true;
            res.postStatus = false;
            res.replyArray=[];
            console.log('Save Exchange',res);
            that.prodManager.saveDataWithCache(this.product['exchangeId'],res);
        });
}catch(e){
    return null;
}
...
_select(){
    ...
    //    Dazzle.loadChangeComponent('change','change-add-product');
        ...
        dzPopup.dialog = this.dialog;
        dzPopup.isFree = true;
        dzPopup.isPurchase = false;
        dzPopup.isExchange = false;
//        dzPopup.postStatus = false;
