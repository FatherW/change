...
class dzElm extends PolymerElement {
    ...
    ready() {
        super.ready();
        this.status = this.shadowRoot.querySelector('.dot');
        // this.ownerId = this.getAttribute('ownerid') || false;
        this.prodManager.getDataByES(this.pid).then(prod=>{
            this.product = prod;
            console.log('Data2',this.pid,this.product);
            this.initStatus();
        });

    }
    initStatus(){
        this.status.setAttribute('class','dot green');
        try {
            if (this.product['isPurchase'])
                this.status.setAttribute('class','dot red');
            else if (this.product['replyArray'].length>0) {
                this.status.setAttribute('class','dot yellow');
                let span = document.createElement('span');
                span.innerHTML =  "("+this.product['replyArray'].length+")";
                this.shadowRoot.querySelector('.status').appendChild(span);
            } 
        } catch(e){
